...
class RepoVectorService:
    ...
    def update_all_in_one_vector(self, file_sha256: str):
        """构造(图片特征向量)与(tag_text拼接ocr_text的特征向量)的加权平均向量，使用Clip模型
        ...
        with self.Session() as session:
            img_vector_mapper = ImgVectorMapper(session)
            img_vector_do = img_vector_mapper.query_by_file_sha256(file_sha256)
            logger.info(f"update_all_in_one_vector: {img_vector_do.file_dir}\\{img_vector_do.file_name}")
            # 获得图片特征向量
            img_vector_pg_str = img_vector_do.img_vec
            img_vector_list = ast.literal_eval(img_vector_pg_str)
            img_vector = np.array(img_vector_list, dtype=np.float32)

            # 计算全部文本的特征向量，注意必须把tag_text放在前面防止被模型512token限制截断
            all_text = StringUtil.concat(img_vector_do.tag_text, ",", img_vector_do.ocr_text)
            all_text_vector = self.chineseClip.embed_text_to_vec(all_text)
            # 加权平均
            alpha = 0.5
            all_in_one_vector = alpha * img_vector + (1 - alpha) * all_text_vector
            # 归一化
            ...
            img_vector_mapper.update_all_in_one_vec_by_file_sha256(file_sha256, all_in_one_vector_pg_str)
